...
Let's start with history. Skepticism has roots in classical antiquity. A trilemma by **Agrippa** (2nd century AD) is one of the earliest formulation of the skeptical challenge. Since every belief is produced by a method or source (say, memory, perception, experiments, divine inspiration, etc.), there 
...
No one of these options seems to be promising.^[Each option, however, could be seen 
as an epistemoliogical strategy: coherentism (embracing circularity), infinitism (embracing infinite regress), foundatioanlism (embracing the fact that some belief are basic).] 
...
not clear how my committments to those methods can be undesrtood as rational. 
...
something more like religious conversation. (p. 45)^[There are two problems here. *Problem 1*: how do we justify the reliability of a method we rely on (say empirical observation)? *Problem 2*: if we decide to change a method of inquiry (say from trusting divine revelation to performing scientific experiments), how can we rationally justify this change?]

The most well-known response to skepticism was given 
by **Descartes** (1596–1650):

> the foundation of knowledge, for Descartes, is our priviledge access to the contents of our own minds...
He looked within himself and found the idea of perfection....So if anythig causes the idea of perfection, it must itself be something perfect...and so a perfect being---God---must exists. And if he exists, and is perfect, then he would not deceive us. So what is clearly and distinctly perceived must really be true. (p. 48)^[This is a very quick 
summary of Descartes' *Meditation on First Philosophy*.]
...
accept your principles, your reassuring me that they are reliable because emplying them tells you so won't impress me. Nor should it. (p. 56)

Here the problem is not so much whether a given method is reliable. Rather, the problem is this: how person $A$ can rationally persuade person $B$, where $A$ follows a method $M$ which $B$ does not follow. Persons $B$ must be rationally persuased about method $M$ by person $A$ via noncircular reasons which $B$ can (and should) recognize as reasons for relying on method $M$.
The skeptic holds there is no way to answer this problem--this is an imposisble task.
...
to the fist lemma of Agrippa's trilemma.
...
around and talk to the Nazis" (p. 69)] Resaons is replaced by persuasion, manipulation, 
...
response to skepticism, what do we do?^[Lynch in his book has an answer. If you are curious, 
