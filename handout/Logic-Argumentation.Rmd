...
\noindent
This definition is just a sketch. We need to fill in the sketch (**V**), 
...
According to logical pluralism, it depends. There is no validity in an absolute and unique sense.^[If logical pluralism is true, there is not just one way in which a conclusion can follow from a set of premises. Does this make reasoning and logic entirely relative and up from  grabs?] 
...
these four *clarity conditions* hold:^[We are often mistaken whether we are in the fog or not. We could think we are not in the fog, but we actually are in it full force. Being in the fog and being aware of that fact is perhaps better than being in the fog without being aware of that fact. Why is that?]
